    ...
    for k in rxn_inputs:
        for component in rxn_inputs[k].components:
            ...
            if component.reaction_role == ReactionRole.CATALYST:
                ...
                row["catalyst_concentration (M)"] = conc.to(ureg.moles / ureg.liters).magnitude
            ...
            if component.reaction_role == ReactionRole.REACTANT:
                ...
                row[f"reactant_{reactants+1}_concentration (M)"] =  conc.to(ureg.moles / ureg.liters).magnitude
                reactants+=1
            if component.reaction_role == ReactionRole.REAGENT:
                ...
                row[f"reagent_concentration (M)"] =  conc.to(ureg.moles / ureg.liters).magnitude
